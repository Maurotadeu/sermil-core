 ...
 * @author Abreu Lopes
 * @since 3.0
 * @version $Id: Ra.java 2472 2014-06-15 09:06:52Z wlopes $
 */
public final class Ra implements Serializable {

   private static final long serialVersionUID = -7712585464456038108L;
   ...
   public Ra(final Long valor) throws SermilException {
      ...
      Ra ra = new Ra.Builder().csm(Integer.parseInt(aux.substring(0, 2))).jsm(Integer.parseInt(aux.substring(2, 5))).sequencial(Integer.parseInt(aux.substring(5, 11))).build();
   ...
   public static String formatRA(Long ra) {
      final String aux = String.format("%012d", ra);
      Ra ral = new Ra.Builder().csm(Integer.parseInt(aux.substring(0, 2))).jsm(Integer.parseInt(aux.substring(2, 5))).sequencial(Integer.parseInt(aux.substring(5, 11))).build();
      return ral.toString();
   }
   ...
   public static class Builder {
      ...
      public Ra build() {
         if (StringUtils.isEmpty(_temp.csm) || StringUtils.isEmpty(_temp.jsm) || StringUtils.isEmpty(_temp.sequencial)) {
            throw new IllegalArgumentException("RA é composto de CSM/JSM/SEQUENCIAL");
         }
