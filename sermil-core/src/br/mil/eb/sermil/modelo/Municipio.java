...
@NamedQueries({
  ...
  //@NamedQuery(name = "Municipio.gruparPorMesoregiao", query = "SELECT m.mesoregiao, COUNT(m) FROM Cidadao c JOIN Municipio m ON c.municipioResidencia.codigo = m.codigo WHERE c.vinculacaoAno = ?1 GROUP BY m.mesoregiao ORDER BY COUNT(m) DESC"),
})
...
public final class Municipio implements Serializable {
  ...
  public String toString() {
    return new StringBuilder(this.getDescricao() == null ? "DESCRICAO" : this.getDescricao())
    .append(" - ")
    .append(this.getUf() == null ? "UF" : this.getUf())
    .toString();
  }
