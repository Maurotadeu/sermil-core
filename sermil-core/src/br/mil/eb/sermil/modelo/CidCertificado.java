...
@NamedQueries({ @NamedQuery(name = "Certificado.cidadaoTemCdi", query = " SELECT c.numero FROM CidCertificado c WHERE c.cidadao.ra = ?1 and c.pk.tipo in(3,4,6) "), })
public final class CidCertificado implements Comparable<CidCertificado>, Serializable {

   private static final long serialVersionUID = 2434088743177264940L;
   ...
   public String toString() {
      return new StringBuilder(this.getPk().getTipo() == null ? "TIPO" : this.getPk().getTipo().toString()).append(" - ").append(this.getPk().getData() == null ? "DATA" : DateFormat.getDateInstance(DateFormat.MEDIUM).format(this.getPk().getData()))
            .toString();
   }
   ...
   public static class PK implements Comparable<CidCertificado.PK>, Serializable {
      ...
      /** serialVersionUID. */
      private static final long serialVersionUID = -7454049645071631567L;
